...
describe('로비 생성 Test 케이스', () => {
    ...
    describe('유효성 에러 케이스', () => {
        it('존재하지 않는 토너먼트 id', async () => {
            ...
            await expect(lobbyService.createLobby(tournament_id, max_player, user_id))
                .rejects.toThrow('해당 토너먼트를 찾을 수 없습니다.')
        ...
        it('IN_PROGRESS (이미 시작한) 토너먼트', async () => {
            ...
            await expect(lobbyService.createLobby(tournament_id, max_player, user_id))
                .rejects.toThrow('이미 시작된 토너먼트입니다.')
    ...
    describe('db 조건 에러 케이스', () => {
        ...
        it('로비 생성 중 db 에러', async () => {
            ...
            await expect(lobbyService.createLobby(tournament_id, max_player, user_id))
                .rejects.toThrow('Lobby creation failed')
        });
        it('로비 생성 중 db 에러', async () => {
            tournament_id = 1;
            tournament_type = 'SEMIFINAL';
        ...
        it('로비 생성 중 방생성자(최초방장) 추가시 db 에러', async () => {
            ...
            // Act & Assert
            await expect(lobbyService.createLobby(tournament_id, max_player, user_id))
                .rejects.toThrow('Player addition failed');
        });
    });

    describe('엣지 케이스', () => {
        it('null / undefined 입력 처리', async () => {
            await expect(lobbyService.createLobby(null, 4, 100))
                .rejects.toThrow();
            
            await expect(lobbyService.createLobby(1, undefined, 100))
                .rejects.toThrow();

            await expect(lobbyService.createLobby(1, 4, null))
                .rejects.toThrow();
        });
